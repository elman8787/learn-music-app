...
class lesson5 : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding = ActivityLesson5Binding.inflate(LayoutInflater.from(this))
    ...
    fun onclick(view: View) {
        ...
        if (button.text.toString() == "do" && text == "1") {
            binding.textview.text = mylist.random()
            val player = MediaPlayer.create(this, R.raw.correctanswer)
            player.start()
            visible()
        } else if (button.text.toString() == "re" && text == "2") {
            binding.textview.text = mylist.random()
            val player = MediaPlayer.create(this, R.raw.correctanswer)
            player.start()
            visible()
        } else if (button.text.toString() == "mi" && text == "3") {
            visible()
            binding.textview.text = mylist.random()
            val player = MediaPlayer.create(this, R.raw.correctanswer)
            player.start()
        } else if (button.text.toString() == "fa" && text == "4") {
            binding.textview.text = mylist.random()
            val player = MediaPlayer.create(this, R.raw.correctanswer)
            player.start()
            visible()
        } else if (button.text.toString() == "sol" && text == "5") {
            binding.textview.text = mylist.random()
            val player = MediaPlayer.create(this, R.raw.correctanswer)
            player.start()
            visible()
        } else if (button.text.toString() == "la" && text == "6") {
            binding.textview.text = mylist.random()
            val player = MediaPlayer.create(this, R.raw.correctanswer)
            player.start()
            visible()
        } else if (button.text.toString() == "si" && text == "7") {
            binding.textview.text = mylist.random()
            val player = MediaPlayer.create(this, R.raw.correctanswer)
            player.start()
            visible()
